...
FILE_INTERVAL_SECONDS = 5  # readsb-hist files are every 5 seconds


def generate_file_urls(base_url: str, file_limit: int) -> list[tuple[str, str]]:
    """
    ...
    results = []
    total_seconds = 0
    max_seconds = 24 * 3600  # one full day
...
def download_aircraft(
    ...
    file_urls = generate_file_urls(base_url, limit)
...
settings = Settings()
bucket = settings.s3_bucket  # reads BDI_S3_BUCKET from .env automatically
