...
def create_relationship(rel: RelationshipCreate) -> dict:
    ...
    with driver.session() as session:
        from_exists = session.run(
            "MATCH (p:Person {name: $name}) RETURN p", name=rel.from_person
        ).single()
        if not from_exists:
            driver.close()
        ...
        to_exists = session.run(
            "MATCH (p:Person {name: $name}) RETURN p", name=rel.to_person
        ).single()
        if not to_exists:
            driver.close()
        ...
        session.run(
            ...
            """,
            from_person=rel.from_person, to_person=rel.to_person
        )
    driver.close()
